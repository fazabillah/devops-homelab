...
Operational reference for the lab1-java stack: Spring Boot screening app, Oracle XE, Splunk, ArgoCD on K3s.
...
| search kubernetes.namespace_name=java-prod
...
| Root cause | Fix |
|---|---|
| Oracle pod not started / initializing | Wait — first boot takes 3–5 min. Watch: `kubectl logs -f oracle-0 -n java-prod` |
| Oracle pod CrashLoopBackOff | `kubectl describe pod oracle-0 -n java-prod` — check events section for cause |
| PVC stuck Pending | `kubectl describe pvc oracle-data-pvc -n java-prod` — local-path provisioner issue |
| Wrong DB_PASSWORD in secret | Recreate: `kubectl create secret generic oracle-db-secret --from-literal=DB_PASSWORD=CorrectPassword -n java-prod --dry-run=client -o yaml \| kubectl apply -f -` then restart app pods |
| App pods have stale secret env | `kubectl rollout restart deployment/prod-java-screening-app -n java-prod` |
...
- `kubectl get pods -n java-prod` shows the app pod restarting (RESTARTS count climbing)
...
kubectl describe pod -n java-prod -l app=java-screening-app
# Look for:
#   Last State: Terminated
#   Reason: OOMKilled

# Step 2: Check recent events for the namespace
kubectl get events -n java-prod --sort-by='.lastTimestamp' | tail -20
# Look for: OOMKilling events from kubelet

# Step 3: Check current memory limit vs request
kubectl get deployment prod-java-screening-app -n java-prod \
...
Edit `java-kustomize/overlays/prod/patch-replicas.yaml` (or add a new memory patch file):

```yaml
# java-kustomize/overlays/prod/patch-memory.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: java-screening-app
spec:
  template:
    spec:
      containers:
...
Add it to `java-kustomize/overlays/prod/kustomization.yaml` under `patches:`, then commit and push. ArgoCD deploys within 3 minutes.
...
- `argocd app get java-app-prod` shows `OutOfSync` or new pods have not rolled out
...
argocd app get java-app-prod --refresh

# Step 2: Check what ArgoCD thinks is out of sync
argocd app diff java-app-prod

# Step 3: Confirm the bot commit actually landed in the overlay
cat java-kustomize/overlays/prod/kustomization.yaml | grep newTag
...
| Root cause | Fix |
|---|---|
| ArgoCD poll hasn't fired yet | `argocd app sync java-app-prod` — triggers immediate sync |
| Bot commit did not push (CI error) | Check GitHub Actions run → `Update image tag in Kustomize overlay` step. Fix and re-push. |
| Sync operation stuck/hung | `argocd app terminate-op java-app-prod` then `argocd app sync java-app-prod` |
| Hook error blocking sync | `argocd app get java-app-prod` → look for failed PreSync/PostSync hook → `argocd app delete-resource java-app-prod --kind Job --name <hook-name>` then re-sync |
...
# App readiness
kubectl rollout status deployment/prod-java-screening-app -n java-prod

# Live logs from app
kubectl logs -f -n java-prod -l app=java-screening-app

# Oracle status
kubectl exec -it oracle-0 -n java-prod -- sqlplus system/YourStrongPassword123@XEPDB1

# ArgoCD all-app status
argocd app list

# Port-forwards for local access
kubectl port-forward svc/prod-java-screening-app 8080:8080 -n java-prod &
